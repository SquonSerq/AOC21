...
def solve_draw_numbers(numbers, decks):
	...
	decks_local = decks
	for num in numbers:
		for deck in range(0, len(decks_local)):
			for i in range(0, 5):
				for j in range(0, 5):
					if decks_local[deck][i][j]['drawn'] == True:
						continue
					
					if int(decks_local[deck][i][j]['number']) == num:
						print('in')
						decks_local[deck][i][j]['drawn'] = True
			win_deck_sum = check_deck(decks_local[deck])
